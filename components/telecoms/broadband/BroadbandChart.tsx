...
function computeStats(
	...
	const fromRecord = (code: string) => {
		...
		if (!r) return null;
		return {
			pctSuperfast: r.pctSuperfast,
			pctUltrafast: r.pctUltrafast,
			pctFullFibre: r.pctFullFibre,
			pctGigabit: r.pctGigabit,
			...
			{!hasData ? (
				...
				<div className="flex items-end justify-between gap-1.5 flex-1">
					...
					<div className="flex gap-1 shrink-0">
						<StatPill
							label="Superfast"
							value={stats!.pctSuperfast}
							isDark={isDark}
						/>
						<StatPill
							label="Ultrafast"
							value={stats!.pctUltrafast}
							isDark={isDark}
						/>
						<StatPill
							label="Gigabit"
							value={stats!.pctGigabit}
							isDark={isDark}
						/>
					</div>
				</div>
