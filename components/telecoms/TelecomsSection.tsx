...
export default function TelecomsSection(props: Props) {
	...
	if (!hasVisibleScalarChart("Telecoms", visibility)) return null;
	return <div className={`space-y-2 border-t ${isDark ? "border-white/10" : "border-gray-200/80"}`}><h3 className={`text-xs font-bold pt-2 ${isDark ? "text-gray-200" : "text-gray-800"}`}>Telecoms</h3><ScalarChartCards group="Telecoms" visibility={visibility} {...props} /></div>;
}
