...
export async function loadQualification(
	...
	const { data } = await parseCsv(await read("education/TS067-2021-1.csv"), { header: true });

	const laData: Record<string, QualificationBreakdown> = {};
	for (const row of data as any[]) {
		const ladCode = pick(row,
			"Lower Tier Local Authorities Code", "geography code", "GeographyCode", "LA Code", "laCode",
		);
		if (!ladCode) continue;

		const catCode = pick(row,
			"Highest level of qualification (8 categories) Code", "Qualification Code", "qualificationCode", "Category Code",
		);
		...
		const count = parseNullableInt(pick(row, "Observation", "Count", "Value", "observation"));
		if (count === null) continue;

		if (!laData[ladCode]) {
			laData[ladCode] = {
				noQualifications: 0, level1: 0, level2: 0, apprenticeship: 0,
				level3: 0, level4Plus: 0, other: 0, total: 0,
			};
		}
