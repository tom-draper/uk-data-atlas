...
export const fetchAndParseGeneralElectionData = async (config: GeneralElectionSourceConfig): Promise<GeneralElectionDataset> => {
    ...
    if (config.requiresHeaderCleaning) {
        ...
        const dataStart = lines.findIndex(line => line.includes(config.fields.onsId));
        ...
        Papa.parse(csvText, {
            ...
            complete: (results) => {
                ...
                for (const row of results.data as any[]) {
                    ...
                    // 3. Extract core data
                    const winningParty = row[config.fields.firstParty]?.trim().toUpperCase() || 'OTHER';
                ...
                resolve({
                    ...
                    name: `General Election ${config.year}`,
                    year: config.year,
