...
export interface CodeMapper {
	getCodeForYear(type: CodeType, code: string, targetYear: YearCode): string | undefined;
	getWardsForLad(ladCode: string, year: YearCode): string[];
	getWardsForConstituency(constituencyCode: string, wardYear: YearCode): string[];
}
...
const emptyCodeMappings = (): CodeMappings => ({
	ward: {}, localAuthority: {}, constituency: {}, lsoa: {}, dataZone: {}, superOutputArea: {},
});
...
export class CodeMapperStore implements CodeMapper {
	...
	getLadForWard = (wardCode: string): string | undefined => {
		...
		for (const equivalentCode of Object.values(this.codeMappings.ward[wardCode] ?? {})) {
	...
	addWardLadMapping = (wardCode: string, localAuthorityCode: string): void => {
		if (wardCode && localAuthorityCode) this.wardToLad[wardCode] = localAuthorityCode;
	};
	...
	addLadWardMapping = (year: YearCode, ladCode: string, wardCodes: string[]): void => {
	...
	addConstituencyWardMappings = (year: YearCode, mappings: Record<string, string[]>): void => {
		if (year) Object.assign((this.constituencyToWards[year] ??= {}), mappings);
	};

	getWardsForConstituency = (constituencyCode: string, wardYear: YearCode): string[] => {
		const direct = this.constituencyToWards[wardYear]?.[constituencyCode];
		if (direct?.length) return direct;
		const currentCode = this.codeMappings.constituency[constituencyCode]?.[2024];
		return currentCode ? this.constituencyToWards[wardYear]?.[currentCode] ?? [] : [];
	};

	addCodeMapping = (type: CodeType, fromCode: string, toYear: YearCode, toCode: string): void => {
	...
	getCodeForYear = (type: CodeType, code: string, targetYear: YearCode): string | undefined =>
		this.codeMappings[type][code]?.[targetYear];

	getAllEquivalentCodes = (type: CodeType, code: string): Array<{ year: YearCode; code: string }> =>
		Object.entries(this.codeMappings[type][code] ?? {}).map(([year, mappedCode]) => ({
			year: Number(year), code: mappedCode,
		}));

	findSourceCodes = (type: CodeType, targetCode: string, targetYear: YearCode): string[] =>
		[...(this.reverseMappings[type][targetCode] ?? [])].filter(
			(sourceCode) => this.codeMappings[type][sourceCode]?.[targetYear] === targetCode,
		);

	getHighlightCodes = (type: CodeType, code: string): Set<string> => {
		const codes = new Set<string>([code]);
		for (const mappedCode of Object.values(this.codeMappings[type][code] ?? {})) codes.add(mappedCode);
		for (const sourceCode of this.reverseMappings[type][code] ?? []) {
			codes.add(sourceCode);
			for (const mappedCode of Object.values(this.codeMappings[type][sourceCode] ?? {})) codes.add(mappedCode);
		}
		return codes;
	};
	...
	clearWardLadMap = (): void => { this.wardToLad = {}; };
	clearLadWardMap = (): void => { this.ladToWards = {}; };
	...
	getMappingCounts = () => {
		...
		return {
			...
			localAuthority: Object.keys(this.codeMappings.localAuthority).length,
