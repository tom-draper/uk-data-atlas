		...
		const loadData = async () => {
			try {
				...
				const response = await fetch(
					withCDN(
						"/data/crime/policeforceareatablesyejune25final.csv",
					),
				);
				if (!response.ok) {
					throw new Error(
				...
				Papa.parse(csvText, {
					...
					complete: (results: any) => {
						try {
							...
							dataRows.forEach((row: string[]) => {
								...
								const record: CrimeRecord = {
									...
									communitySafetyPartnershipName:
										row[3]?.trim() || "",
									totalRecordedCrime: parseNumberStrict(
										row[6],
									),
									violenceAgainstPerson: parseNumberStrict(
										row[7],
									),
									homicide: parseNumberStrict(row[8]),
									deathSeriesInjuryUnlawfulDriving:
										parseNumberStrict(row[9]),
									violenceWithInjury: parseNumberStrict(
										row[10],
									),
									violenceWithoutInjury: parseNumberStrict(
										row[11],
									),
									stalkingHarassment: parseNumberStrict(
										row[12],
									),
									...
									residentialBurglary: parseNumberStrict(
										row[17],
									),
									nonResidentialBurglary: parseNumberStrict(
										row[18],
									),
									...
									otherTheftOffences: parseNumberStrict(
										row[23],
									),
									criminalDamageArson: parseNumberStrict(
										row[24],
									),
									drugOffences: parseNumberStrict(row[25]),
									possessionWeapons: parseNumberStrict(
										row[26],
									),
									publicOrderOffences: parseNumberStrict(
										row[27],
									),
									miscellaneousCrimes: parseNumberStrict(
										row[28],
									),
								};
							...
							setError(
								parseErr.message || "Error parsing crime data",
							);
							setLoading(false);
						}
					},
