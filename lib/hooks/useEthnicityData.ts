...
const calculateResults = (
	...
	for (const [localAuthorityCode, parentCategories] of Object.entries(
		localAuthorityData,
	)) {
		let maxPopulation = 0;
...
const parseEthnicityData = async (): Promise<{
	...
	return new Promise((resolve, reject) => {
		Papa.parse(csvText, {
			...
			dynamicTyping: false,
			complete: (results) => {
				const localAuthorityData: Record<
					string,
					Record<string, any>
				> = {};
				...
				for (const row of results.data as any[]) {
					...
					const fullName =
						row["Ethnic group (20 categories)"]?.trim() || "";
					const { parent, subcategory } =
						parseEthnicityName(fullName);
