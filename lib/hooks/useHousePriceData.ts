...
const parseHousePriceData = async (): Promise<
	Record<string, WardHousePriceData>
> => {
	...
	const dataStart = lines.findIndex((line) =>
		line.includes("Local authority code"),
	);
		...
		Papa.parse(cleanedCsv, {
			...
			complete: (results) => {
				...
				for (const row of results.data as any[]) {
					...
					wardData[wardCode] = {
						localAuthorityCode:
							row["Local authority code"]?.trim() || "",
						localAuthorityName:
							row["Local authority name"]?.trim() || "",
						wardCode,
