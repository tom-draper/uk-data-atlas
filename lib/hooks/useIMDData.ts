	...
	return useDataLoader<IMDDataset>(async () => {
		...
		for (const row of data as any[]) {
			...
			records[lsoaCode] = {
				...
				imdRank: parseNumInt(row["Index of Multiple Deprivation (IMD) Rank (where 1 is most deprived)"]),
				imdDecile: parseNumInt(row["Index of Multiple Deprivation (IMD) Decile (where 1 is most deprived 10% of LSOAs)"]),
