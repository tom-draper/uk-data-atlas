...
describe("dataset registry metadata", () => {
	it("reads catalogue metadata from the exported definition value", () => {
		expect(catalogueMetadata("example.ts", {
	...
	it("reads every chart from a multi-chart definition", () => {
		expect(chartMetadata("example.ts", {
			exampleDefinition: {
				type: "example",
				chart: { key: "example-current", componentPath: "@/components/Current" },
				charts: [
					{ key: "example-current", componentPath: "@/components/Current" },
					{ key: "example-history", componentPath: "@/components/History" },
				],
			},
		})).toEqual({
	...
	it("rejects incomplete chart metadata", () => {
		expect(() => chartMetadata("example.ts", {
			exampleDefinition: { type: "example", chart: { key: "example" } },
