...
describe("aggregateUnemployment", () => {
	...
	it("averages each year over the areas reporting a rate that year", () => {
		const result = aggregateUnemployment(features(["E1", "E2"]), CODE_KEY, dataset);
	...
	it("omits a year no covered area reports", () => {
		const result = aggregateUnemployment(features(["E2"]), CODE_KEY, dataset);
		expect(result?.rates).toEqual({ 2022: 6 });
	});

	it("returns null when no covered area has a record", () => {
		expect(aggregateUnemployment(features(["missing"]), CODE_KEY, dataset)).toBeNull();
	});
});
...
describe("aggregateHousePrices", () => {
	...
	it("averages the headline price over the wards priced in 2023", () => {
		const result = aggregateHousePrices(features(["E1", "E2", "E3"]), CODE_KEY, data);
	...
	it("reports a zero average when no covered ward has a price", () => {
		const result = aggregateHousePrices(features(["missing"]), CODE_KEY, data);
		expect(result).toEqual({ averagePrice: 0, wardCount: 0, averagePrices: {} });
	});
});

describe("aggregateCrime", () => {
	const data = { E1: { totalRecordedCrime: 100 }, E2: { totalRecordedCrime: 300 } } as any;
...
describe("aggregateCustomDataset", () => {
	...
	it("keeps a zero value in the average", () => {
		expect(
			aggregateCustomDataset(features(["E1", "E2"]), CODE_KEY, { E1: 0, E2: 10 }),
		).toEqual({ count: 2, average: 5 });
	});

	it("reports zero when nothing is covered", () => {
		expect(aggregateCustomDataset(features(["missing"]), CODE_KEY, {})).toEqual({
			count: 0,
			average: 0,
		});
	});
