...
type ManifestEntry = {
	type: string;
	output: string;
	source: { name: string; sourceUrl: string; licence: string; licenceUrl: string };
	summary: { datasetCount: number; dataRecordCount: number; boundaryYears: number[] };
...
const manifestByType = new Map(manifest.datasets.map((entry) => [entry.type, entry]));
	...
	for (const definition of CHART_DATASET_DEFINITIONS) {
		describe(definition.type, () => {
			it("has complete provenance and registered chart cards", () => {
				...
				const chartKeys = getChartDefinitions(definition).map((chart) => chart.key);
			...
			it("has a validated, provenance-matched compiled artifact", () => {
				...
				const summary = validatePrecompiledDataset(definition, compiled.data as never);
