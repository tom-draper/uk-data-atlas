...
describe("chart dataset registry contract", () => {
	...
	it("assigns every registry chart to a displayed group", () => {
		...
		expect(
			new Set(CHART_GROUPS.map((definition) => definition.group)),
		).toEqual(chartGroups);
	});
	...
	for (const definition of CHART_DATASET_DEFINITIONS) {
		describe(definition.type, () => {
			it("has complete provenance and registered chart cards", () => {
				...
				expect(
					() => new URL(definition.source.sourceUrl),
				).not.toThrow();
				expect(
					() => new URL(definition.source.licenceUrl),
				).not.toThrow();
			...
			if (definition.map) {
				it("has a valid choropleth encoding", () => {
					const { colorRange, legend, valueFor, valueKey } =
						definition.map!;
				...
				it("registers a bespoke map renderer", () => {
					expect(definition.mapRenderer?.getOptions).toBeTypeOf(
						"function",
					);
					expect(definition.mapRenderer?.render).toBeTypeOf(
						"function",
					);
				});
			}
