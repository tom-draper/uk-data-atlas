...
import { loadWIMD, publishedWIMDRanks } from "@/lib/data/wimd/loader";
...
describe("loadWIMD", () => {
	it("reads the published rank and decile rather than re-ranking scores", async () => {
		const datasets = await loadWIMD(async () => scoresCsv, ranksXml);
	...
	it("refuses an LSOA with no published rank", async () => {
		const missing = ranksXml
			...
			.join("\n");
		await expect(loadWIMD(async () => scoresCsv, missing)).rejects.toThrow(
