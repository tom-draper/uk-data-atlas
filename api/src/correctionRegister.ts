...
export type CorrectionRecord = {
	...
	evidence: Array<{ kind: "method" | "catalogue"; href: string; note: string }>;
...
export const correctionRecords: CorrectionRecord[] = [
	{
		...
		scope: {
			measureIds: ["*"],
			fields: ["value", "confidenceInterval.lower", "confidenceInterval.upper"],
			request: "GET /v1/data/{measure-id}?units=canonical",
		},
		change: {
			source:
				"The default response preserves each publisher's value, label and interval bounds.",
			served:
				"On the explicit canonical request, each numeric value and any publisher-supplied interval bound is multiplied by the measure's declared scale; the response names the resulting unit and calculation.",
			sourceArtifactsChanged: false,
		},
		evidence: [
			{
