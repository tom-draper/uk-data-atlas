	...
	return (
		...
		>
			...
			<P>
				...
				Requests` as a [problem document](/docs/errors), with
			...
			<List
				items={[
					"Send back the `ETag` you were given. A `304 Not Modified` still costs a request, but it costs no bandwidth. See [Caching](/docs/caching).",
					"Take a whole table rather than a row at a time: [bulk downloads](/docs/formats) serve an entire partition or lookup in one request.",
					"Pin an [Atlas release](/docs/concepts/releases#pinning). A pinned resource is immutable and can be cached for good, so you never re-fetch it.",
