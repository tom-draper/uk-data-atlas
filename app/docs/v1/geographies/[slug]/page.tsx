...
export default async function GeographyPage({ params }: { params: Params }) {
	...
	return (
		<DocPage
			...
			trail={[
				{ label: "Geographies", href: "/docs/v1/geographies" },
				{
					label: group?.title ?? "",
					href: `/docs/v1/geographies#${content.group}`,
				},
			]}
			title={content.title}
			lede={content.intro}
		...
		>
			...
			{latest.source.licence && (
				<P>
					{`Boundaries are published by ${latest.source.publisher} under the ${latest.source.licence.name}. [Get attribution text](/docs/v1/reference/governance/attribution) gives the credit line for a map.`}
				</P>
			)}
